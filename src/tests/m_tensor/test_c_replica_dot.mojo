...
from src.m_tensor.dynamic_tensor import create_dynamic_tensor_from_data, create_dynamic_tensor, dense_tensor_dot
...
fn test_dense_tensor_dot() raises:
    ...
    with DeviceContext() as ctx:
        ...
        var A = create_dynamic_tensor_from_data[DType.float32](ctx, A_data, List[Int](2, 3)^)
        print("A:")
        A.print_tensor(ctx)
        ...
        var B = create_dynamic_tensor_from_data[DType.float32](ctx, B_data, List[Int](3, 2)^)
        print("B:")
        B.print_tensor(ctx)
        
        # C = A @ B (should be 2x2)
        # Expected: [[58, 64], [139, 154]]
        var C = create_dynamic_tensor[DType.float32](ctx, List[Int](2, 2)^, init_value=Scalar[DType.float32](0.0))
        ...
        var M = create_dynamic_tensor_from_data[DType.float32](ctx, M_data, List[Int](2, 3)^)
        print("M:")
        M.print_tensor(ctx)
        
        # v = [10, 20, 30]  (3,)
        var v_data = List[Float32](10, 20, 30)
        var v = create_dynamic_tensor_from_data[DType.float32](ctx, v_data, List[Int](3)^)
        print("v:")
        v.print_tensor(ctx)
        
        # result = M @ v (should be 2,)
        # Expected: [140, 320]
        var result = create_dynamic_tensor[DType.float32](ctx, List[Int](2)^, init_value=Scalar[DType.float32](0.0))
        print("result:")
        ...
        var X = create_dynamic_tensor_from_data[DType.float32](ctx, X_data, List[Int](3, 3)^)
        print("X:")
        X.print_tensor(ctx)
        ...
        var Y = create_dynamic_tensor_from_data[DType.float32](ctx, Y_data, List[Int](3, 3)^)
        print("Y:")
        Y.print_tensor(ctx)
        
        # Z = X @ Y (should be 3x3)
        # Expected: [[1, 2, 3], [8, 10, 12], [21, 24, 27]]
        var Z = create_dynamic_tensor[DType.float32](ctx, List[Int](3, 3)^, init_value=Scalar[DType.float32](0.0))
        ...
        var P = create_dynamic_tensor_from_data[DType.float32](ctx, P_data, List[Int](2, 2)^)
        print("P:")
        P.print_tensor(ctx)
        ...
        var Q = create_dynamic_tensor_from_data[DType.float32](ctx, Q_data, List[Int](2, 2)^)
        print("Q:")
        Q.print_tensor(ctx)
        ...
        var R = create_dynamic_tensor[DType.float32](ctx, List[Int](2, 2)^, init_value=Scalar[DType.float32](0.0))
        ...
        var u = create_dynamic_tensor_from_data[DType.float32](ctx, u_data, List[Int](4)^)
        print("u:")
        u.print_tensor(ctx)
        
        # w = [5, 6, 7, 8]
        var w_data = List[Float32](5, 6, 7, 8)
        var w = create_dynamic_tensor_from_data[DType.float32](ctx, w_data, List[Int](4)^)
        print("w:")
        w.print_tensor(ctx)
        
        # dot_result = u · w (should be scalar, represented as 0-d tensor)
        # Expected: 1*5 + 2*6 + 3*7 + 4*8 = 5 + 12 + 21 + 32 = 70
        var dot_result = create_dynamic_tensor[DType.float32](ctx, List[Int](1)^, init_value=Scalar[DType.float32](0.0))
